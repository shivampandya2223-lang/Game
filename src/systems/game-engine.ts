...
export class GameEngine {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.world.gravity.set(0, -18, 0);   // stronger gravity feels better on dunes
    ...
    this.lighting.setTimeOfDay(14); // start at 2 pm — golden desert afternoon
  ...
  private tick(dt: number): void {
    ...
    // Dust cloud when driving fast
    this.dustTimer += dt;
    if (vs.speed > 8 && this.dustTimer > 0.07) {
      ...
      const dustCount = Math.floor(Math.min(vs.speed / 20, 6));
      this.particles.emitDust(dustPos, vs.velocity, dustCount);
      this.dustTimer = 0;
    }

    // Ambient wind streaks (occasional)
